...
用户可见品牌：
- 标题：`企业价值引擎｜<案例主题>`
- 图片品牌：`JN 企业价值引擎` / `jn-value-engine`
...
2. 顶部约 15% 为通栏行业实景 Hero，叠加深蓝渐变；左上为金色方框 `JN` + `企业价值引擎` + 小字 `从问题到方案 · 从洞察到价值`；中左为案例主标题与一行问题副标题；右侧为与最终结论一致的白色手写金句。
...
8. 底部为深蓝 Footer：左侧 `JN 企业价值引擎｜让复杂的商业决策变简单`，右侧 `jn-value-engine｜从洞察到行动，创造可衡量的企业价值`。
...
检查顺序为完整文字→完整图片；图片与文字结论一致；图片符合 JN 白底咨询母版，含 A/B/C、阶段路线、关键指标、4 Gate、下一步和 JN 品牌；标题前、文字与图片之间、图片之后均无主动输出的过程说明或尾声。通过后才 `COMPLETE`。
