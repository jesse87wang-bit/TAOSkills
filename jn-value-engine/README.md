...
v3.5.0 在强制状态机基础上增加“用户可见输出纯净锁”：用户只看到正式01–09文字分析，随后直接看到 JN 企业价值路线图；不再看到 Skill 调用、读取、校验、解锁、生成进度或完成复述。
...
仓库中历史版本遗留的文件可以用于研究与追溯，但不得覆盖 v3.5.0 的 `SKILL.md`。
...
3. 校验通过后才解锁图片；
4. 紧接着生成并显示同源 JN 企业价值路线图；图片后立即结束，不加尾声。

路线图固定采用“纺织企业智能化改造”白底高密度咨询报告母版：深蓝行业 Hero + 白底卡片区 + 金色编号，包含决策结论、现状、真正问题、关键矛盾、潜在机会、A/B/C、阶段路线、关键指标、4个决策闸门、未来1–2周行动和 JN Footer。
...
ChatGPT 官方 Skills 支持以 `SKILL.md` + 支持资源构成可复用工作流。推荐直接安装 **v3.5.0 clean bundle**；详细说明见 [`INSTALL.md`](./INSTALL.md)。
